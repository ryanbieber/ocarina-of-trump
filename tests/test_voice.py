...
class VoiceGenerationTests(unittest.TestCase):
    def test_estimate_uses_n64_vadpcm_frame_ratio(self) -> None:
        estimate = estimate_script(load_manifest(), 150, cues=load_voice_cues())
        self.assertEqual(estimate["pages"], 177)
        self.assertEqual(estimate["cues"], 6)
    ...
    def test_voice_outputs_cover_every_page(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            export_voice_script(entries, script, cues=cues)
            generate_voice_map(entries, source)
            payload = json.loads(script.read_text(encoding="utf-8"))
            expected = sum(len(entry.pages) for entry in entries) + len(cues)
            self.assertEqual(len(payload["clips"]), expected)
            self.assertTrue(payload["disclosure"].startswith("Fictional AI-generated"))
            self.assertEqual(
                source.read_text(encoding="utf-8").count("{ 0x"),
                expected - len(cues),
            )
            self.assertTrue(any(clip.get("cue") == "call" for clip in payload["clips"]))
